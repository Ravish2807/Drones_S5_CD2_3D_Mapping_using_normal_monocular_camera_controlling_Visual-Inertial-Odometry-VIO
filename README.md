<p align="center">
  <img src="assets/amrita_logo.png" alt="Amrita Vishwa Vidyapeetham" width="430">
</p>
...
$$
\mathbf{M}_d = -K_R\mathbf{e}_R - K_\Omega\mathbf{e}_{\Omega} + \Omega \times (J\Omega)
- J\left(\hat\Omega R^TR_d\Omega_d - R^TR_d\dot\Omega_d\right)
$$
...
```mermaid
flowchart TB
    M[MATLAB Live Scripts\nToy Model/*.mlx] --> R[Design/Validation Inputs]
    A[ArduPilot SITL] <--> G[Gazebo Simulation]
    A -->|DDS topics| C[ROS 2 drones_controller]
    C -->|/ap/v1/cmd_vel| A
    C -->|diagnostics| V[RViz / Topic Echo / Logging]
    R --> C
```

- **State Source (implemented):** `/ap/v1/pose/filtered`, `/ap/v1/twist/filtered`.
- **Controller (implemented):** `controller_node.py` + `so3_controller.py` + `state_adapter.py`.
- **Command Output (implemented):** `/ap/v1/cmd_vel` (`geometry_msgs/msg/TwistStamped`).
- **Auxiliary (implemented):** keyboard teleop node (`teleop.py`) and parameter file (`config/controller.yaml`).
- **Estimator / mapping nodes (not present as standalone code in this repo):** treated as external/planned pipeline components.
...
### Placeholder slots for curated figures (to be replaced by team)

```md
![Trajectory Tracking Plot](Results/<replace_with_tracking_plot>.png)
![Attitude Error / Control Signal Plot](Results/<replace_with_control_plot>.png)
![3D Mapping / Reconstruction Output](Results/<replace_with_mapping_output>.png)
```

Video reference provided by the team: [Project Video](https://www.youtube.com/watch?v=gmHnhWYfuW0&t=1s)
...
- **SO(3):** Special Orthogonal Group used for globally consistent 3D attitude control.
- **Quaternion:** Singular-free orientation representation used for conversion and command publication.
- **VIO:** Visual-Inertial Odometry combining camera and IMU estimates (project scope).
- **SLAM:** Simultaneous Localization and Mapping for localization + map building (project scope).
- **ROS 2 Humble:** Middleware used for topic communication and node orchestration.
- **DDS:** Underlying ROS 2 communication layer used in this architecture.
- **ArduPilot SITL:** Software-in-the-loop vehicle/sensor simulation backend.
- **Gazebo:** Physics-based simulator for multirotor environment interaction.
- **MAVLink:** Standard MAV communication protocol relevant to ArduPilot ecosystems.
- **MATLAB:** Live-script-based modeling and control experimentation support.
